...
function PeriodIcon({ period }: { period?: string }) {
  ...
  if (p.includes("morning") || p.includes("fajr")) {
    // Sunrise icon
    return (
  ...
  if (p.includes("evening") || p.includes("night")) {
    // Sunset icon
    return (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-sage">
...
function StopItem({ stop, period }: { stop: Stop; period?: string }) {
  return (
    ...
    return (
      <CollapsibleBlock title="The route" level="h2">
        <div className="space-y-0 pl-2">
          {stops.map((stop, i) => (
            <StopItem key={i} stop={stop} period={stop.time} />
          ))}
        </div>
      </CollapsibleBlock>
    );
                ...
                <div key={period}>
                  ...
                  <div className="space-y-0 pl-2">
                    {periodStops.map(({ stop, origIndex }) => (
                      <StopItem key={origIndex} stop={stop} period={period} />
                    ))}
                  </div>
                </div>
              ))}
